if SERVER then
    ...
    hook.Add("EntityTakeDamage", "HitmarkerDetector", function(ply, dmginfo)
        ...
        if (IsValid(att) && att:IsPlayer() && att ~= ply) then
            if (ply:IsPlayer() or ply:IsNPC() or ply:IsNextBot()) then
                if (ply:IsPlayer() && ply:Armor() > 0) then
            ...
            if (ply:GetModel() == "models/props_c17/oildrum001_explosive.mdl" or ply:GetModel() == "models/props_junk/gascan001a.mdl" or ply:GetModel() == "models/props_junk/propane_tank001a.mdl") then
                net.Start("DrawHitMarker")
    ...
    hook.Add("entity_killed", "HitmarkerKillDetector", function(data)
        ...
        if (IsValid(att) && att:IsPlayer() && att ~= ply) then
            ...
            if (ply:IsPlayer() or ply:IsNPC() or ply:IsNextBot()) then
                if (ply:IsPlayer() && ply:LastHitGroup() == HITGROUP_HEAD) then
...
if CLIENT then
    ...
    net.Receive("ArmorIsZero", function(len, ply)
        ...
        if (allowPlayBreak && not isplayed) then
    ...
    hook.Add("HUDPaint", "HitmarkerDraw", function()
        ...
        if (DrawHitM == true && realism ~= 1) then
            -- Default icon
