...
if SERVER then
    ...
    -- Bullet missed you
    hook.Add("EntityFireBullets", "GWZ_ShotMissed", function(attacker, f)
        if AllowIndicator() then
            for i, victim in pairs(player.GetAll()) do
                if (AllowIndicator(victim) and !attacker == victim and IsValid(attacker) and IsValid(victim)) then
                    ...
                    if (distToSqr < (inflictor.Distance + 256) ^ 2) then
                        ...
                        if (missToSqr <= 2025 && distToSqr <= 90000 || distToSqr > 90000 && missToSqr <= 8100 / (distToSqr / 5625)) then
                            local trdata = {}
                            trdata.start = inflictor.Src
...
if CLIENT then
    ...
    hook.Add("HUDPaint", "GWZ_ShowIndicator", function()
        if !GetConVar("gwz_hud_enable"):GetBool() || !GetConVar("gwz_hud_enable_attackindicator"):GetBool() || !GetConVar("cl_drawhud"):GetBool() then return end
